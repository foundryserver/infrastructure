...
/interface ethernet
set [ find default-name=ether13 ] comment="OOB Configuration Port" name=ether13_oob
set [ find default-name=sfp-sfpplus1 ] comment="Downlink Trunk Port To Switch" name=sfpplus1_CRS317-1
/interface vlan
...
add comment="Public IP Vlan - Beanfield" interface=sfpplus1_CRS317-1 name=vlan40 vlan-id=40
add comment="Public IP v6 Vlan - Beanfield" interface=sfpplus1_CRS317-1 name=vlan41 vlan-id=41
...
/ip address
add address=10.90.90.2/24 comment="OOB IP Address" interface=ether13_oob network=10.90.90.0
add address=10.20.20.1/24 comment="VM Vlan IP Address" interface=vlan20 network=10.20.20.0
add address=10.20.10.1/24 comment="Mgmt Vlan IP Address" interface=vlan10 network=10.20.10.0
add address=192.168.0.1/16 comment="VM Vlan IP Address" interface=vlan20 network=192.168.0.0
add address=199.45.150.6/28 comment="Public Router Address" interface=vlan40 network=199.45.150.0
add address=199.45.150.2/28 comment="K8S Ip Address #1" interface=vlan40 network=199.45.150.0
add address=199.45.150.9/28 comment="Development K8S Ip Address" interface=vlan40 network=199.45.150.0
/ip dhcp-server network
add address=192.168.0.0/16 dns-server=192.168.0.1,10.20.20.1,1.1.1.1,8.8.8.8 domain=vm.lan gateway=192.168.0.1
/ip dns
...
add address=10.20.20.199 comment="LXC Nodeport IPs" name=nodeport1.vm.local type=A
...
add address=192.168.0.199 comment="vm nodeport dns" name=nodeport2.vm.lan type=A
...
add action=accept chain=input comment="Allow Est, Related, & Untracked" connection-state=established,related,untracked
add action=accept chain=input comment="Allow ICMP" protocol=icmp
add action=accept chain=input comment="Allow from OOB Subnet" in-interface=ether13_oob src-address=10.90.90.0/24
add action=accept chain=input comment="Allow dhcp" dst-port=67 in-interface=vlan20 protocol=udp
add action=accept chain=input comment="BGP Rule" dst-port=179 in-interface=vlan20 protocol=tcp
add action=accept chain=forward comment="Allow Est, Related & Untracked" connection-state=established,related,untracked
add action=accept chain=forward dst-address=10.20.20.66 in-interface=vlan40 log-prefix=NATTED--> out-interface=vlan20 protocol=tcp
add action=accept chain=forward dst-address=10.20.20.61 in-interface=vlan40 log-prefix=NATTED--> out-interface=vlan20 protocol=tcp
add action=drop chain=forward comment="Mgmt Vlan" dst-address-list="Mgmt List" in-interface=!vlan10
add action=drop chain=forward comment="VM Vlan" dst-address-list="VM List" in-interface=!vlan20
add action=accept chain=forward out-interface=vlan40 src-address-list="VM List"
add action=accept chain=forward out-interface=vlan40 src-address-list="Mgmt List"
add action=accept chain=forward disabled=yes out-interface=*F src-address-list="OOB List"
add action=accept chain=forward out-interface=vlan40 src-address-list="OOB List"
add action=drop chain=forward comment="Drop All Remaining Traffic" log-prefix="FORWARD DROP -->>"
add action=accept chain=forward disabled=yes
add action=accept chain=input comment="Allow udp dns vlan 10" dst-port=53 protocol=udp src-address-list="Mgmt List"
add action=accept chain=input comment="Allow udp dns vlan 40" dst-port=53 protocol=udp src-address-list="Public List"
add action=accept chain=input comment="Allow udp dns vlan 20" dst-port=53 protocol=udp src-address-list="VM List"
add action=accept chain=input comment="Allow tcp dns vlan 10" dst-port=53 protocol=tcp src-address-list="Mgmt List"
add action=accept chain=input comment="Allow tcp dns vlan 40" dst-port=53 protocol=tcp src-address-list="Public List"
add action=accept chain=input comment="Allow tcp dns vlan 20" dst-port=53 protocol=tcp src-address-list="VM List"
add action=accept chain=input comment="Allow tcp dns OOB" dst-port=53 protocol=tcp src-address-list="OOB List"
add action=accept chain=input comment="Allow udp dns OOB" dst-port=53 protocol=udp src-address-list="OOB List"
add action=drop chain=input comment="Drop All Remaining Traffic"
/ip firewall nat
add action=dst-nat chain=dstnat comment="k8s ip nat" dst-address=199.45.150.2 log-prefix="METALLB/Beanfield -->" to-addresses=10.20.20.66
add action=dst-nat chain=dstnat comment="development k8s ip nat" dst-address=199.45.150.9 log-prefix=METALLB---Dev--> to-addresses=10.20.20.61
add action=src-nat chain=srcnat out-interface=vlan40 src-address=10.20.20.66 to-addresses=199.45.150.2
add action=src-nat chain=srcnat out-interface=vlan40 src-address=10.20.20.61 to-addresses=199.45.150.9
add action=masquerade chain=srcnat comment="Allow Outbound Connections from Lan" out-interface=vlan40
/ip route
add disabled=no distance=1 dst-address=0.0.0.0/0 gateway=199.45.150.1 routing-table=main scope=30 suppress-hw-offload=no target-scope=10
/ipv6 route
add comment="Beanfield Default Route" disabled=no dst-address=::/0 gateway=2606:880::10 routing-table=main suppress-hw-offload=no
/ip service
...
/ipv6 address
add address=2606:880::11/127 advertise=no comment="BF PtP Address" interface=vlan40
add address=2606:880:0:1::1/72 advertise=no comment="Foundry Gateway Address" interface=vlan41
...
add action=drop chain=input comment="Drop all not coming from LAN" in-interface=!vlan10
...
add action=drop chain=prerouting comment="Drop non global from WAN" in-interface=vlan40 src-address-list=not_global_ipv6
add action=drop chain=prerouting comment="Drop non global from WAN" in-interface=vlan41 src-address-list=not_global_ipv6
...
add as=64512 connect=yes disabled=no listen=yes local.address=10.20.20.1 .role=ibgp name=k8s-node-px2 remote.address=10.20.20.104/32 .as=64512 router-id=10.20.20.1 routing-table=main
/system clock
...
/tool e-mail
set from="" port=443 server=mail.smtp2go.com tls=yes user=admin@foundryserver.com
/tool romon
set enabled=yes
